...
//#define FileName "test100M.pdf"
//#define FilePath "/media/sd-mmcblk0p1/test100M.pdf"

extern FILE *fp_curfile_t;
...
int Request_collect(int sockfd,const struct sockaddr_in *addr,int len)
{
	...
	chksum(buf,28);
	My_Printf(dug_3g,"1223Request_collect \n");
...
int Request_upfile(FILE *fp,int sockfd,const struct sockaddr_in *addr,int len)
{
	...
	memcpy(&buf[2],&DeviceID,4);
	memcpy(&buf[6],&DataLen,2);
	memcpy(&buf[8],&filesize,8);
	memcpy(&buf[12],cur3gfilename,strlen(cur3gfilename));

	chksum(buf,DataLen+8);
	sendUdpMsg(buf,DataLen+9,MSGID_SERVER_TO_EDAS_RESP_UPLOAD_FILE);
	My_Printf(dug_3g,"Request_upfile ,len:%d\n",len);
	return 1;
} 
	...
	{	
		if(curMsgstate.state == MsgState_snd)
		{
			...
			{				
				...
				{
					is3gvalid = 0;
					My_Printf(dug_3g,"is3gvalid is %d\n",is3gvalid);
				}
...
void doRspUploadFileRes( MsgInfo *msg)
{
	...
	if(fileTransState == REQUEST_UPFILE)
	{
		...
		{
			memcpy(&OffsetList[0],&(msg->ucBuff[9]),4);
			OffsetCnt = 1;
			OffsetState = 1;
...
void saveUploadSuccessFileList(char *filePathName,int size)
{
	...
	fflush(fp);
	fclose(fp);
	My_Printf(dug_3g,"saveUploadSuccessFileList :%s \n",filePathName);
}
...
void doRspCheckUploadData( MsgInfo *msg)
{
	...
	{			
		...
		{		
			case 1:
			case 3:
				memcpy(&OffsetCur,&(msg->ucBuff[9]),4);
			...
			case 2:
				...
				for(i=0;i<OffsetCnt;i++)
				{
					memcpy(&OffsetList[i],&(msg->ucBuff[9+4*i]),4);
					fileTransState = UPLOADING_FILE;	
				}	
...
void doRspReqDownloadCfg( MsgInfo *msg)
{
	...
	memcpy(&g_FileSizeofCfg,&(msg->ucBuff[8+4]),4);
...
void doRspCheckCfgFile( MsgInfo *msg)
{
	...
	if(respCode == 0x01)
	{
		...
		My_Printf(dug_sys,"update_EDAS_P_CFG.ers_reboot!\n");
		sleep(1);
		system("reboot");
	}
}
...
void task_handle_msg()
{
	...
	while(1)
	{
		if(OK == DeQueue(&msg_queue,&msg))
		{
			...
			for(i=0;i<msg.len;i++)
			{
				My_Printf(dug_3g," %02x",msg.ucBuff[i]);
			}
			My_Printf(dug_3g,"\n");
...
int checkdata(unsigned char *buf,unsigned int n)
{
	...
	{
		My_Printf(dug_3g,"3g rcv check error\n");
		return 0;
	}
}
void task_udprcv()
