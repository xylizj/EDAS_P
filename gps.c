...
unsigned char gps_info_buff[GPS_INFO_BUF_LEN];
...
bool process_gps_ser_buf(char *gps_ser_buf, int len)
{
	int i;
	...
	{			
		...
		{
			if( gps_index < GPS_INFO_BUF_LEN )
			{
				gps_info_buff[gps_index] = gps_ser_buf[i];
				gps_index++;
				if(gps_ser_buf[i] == ',')
				{
					if(commaIndex < 20)
				...
				if(gps_ser_buf[i]== '\n')
				{
					if(('M' == gps_info_buff[4]) && ('C' == gps_info_buff[5]))
					{
						if('A'==gps_info_buff[commaLocation[1]+1])
						{
							if(g_rtc_state == 0)
							{										
								memset(gps_utctime,0,15);										
								memcpy(&gps_utctime[0],&gps_info_buff[commaLocation[0]+1],commaLocation[1]-commaLocation[0]-1);//utc_time
								memset(gps_utcdate,0,15);										
								memcpy(&gps_utcdate[0],&gps_info_buff[commaLocation[8]+1],commaLocation[9]-commaLocation[8]-1);//utc_time
							...
							if(0x00 == (gpsflg & 0x01))
							{
								...
								memcpy(&gps_latitude[0],&gps_info_buff[commaLocation[2]+1],commaLocation[3]-commaLocation[2]-1);//latitude			  
							}
							if(0x00 == (gpsflg & 0x02))
							{
								memset(gps_longitude,0,15); 									
								memcpy(&gps_longitude[0],&gps_info_buff[commaLocation[4]+1],commaLocation[5]-commaLocation[4]-1);//longitude			
							}
							if(0x00 == (gpsflg & 0x04))
							{
								memset(gps_speed,0,15); 									
								memcpy(&gps_speed[0],&gps_info_buff[commaLocation[6]+1],commaLocation[7]-commaLocation[6]-1);//speed
							}
							gpsflg = (gpsflg|0x07);
						}
					}
					if(('G' == gps_info_buff[4]) && ('A' == gps_info_buff[5]))
					{
						flagGA = gps_info_buff[commaLocation[5]+1];
						switch(flagGA)
						{
							case '1':
							case '2':
							case '4':
								if(0x00 == (gpsflg & 0x08))
								{
									...
									memcpy(&gps_height[0],&gps_info_buff[commaLocation[8]+1],commaLocation[9]-commaLocation[8]-1);//height
								}
								if(0x00 == (gpsflg & 0x01))
								{											
									memcpy(&gps_latitude[0],&gps_info_buff[commaLocation[1]+1],commaLocation[2]-commaLocation[1]-1);//latitude			  
								}
								if(0x00 == (gpsflg & 0x02))
								{											
									memcpy(&gps_longitude[0],&gps_info_buff[commaLocation[3]+1],commaLocation[4]-commaLocation[3]-1);//longitude	 
								}
								gpsflg = (gpsflg|0x0B);
								break;
							default:
					...
					if(('T' == gps_info_buff[4]) && ('G' == gps_info_buff[5]))
					{
						if(0x00 == (gpsflg & 0x04))
						{
							memset(gps_speed,0,15); 								
							memcpy(&gps_speed[0],&gps_info_buff[commaLocation[4]+1],commaLocation[5]-commaLocation[4]-1);//speed
							gpsflg = gpsflg & 0x04;
						}
					}							
				}
